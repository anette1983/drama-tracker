...
| T06 | E2   | Add NestJS backend config and env contract            | P0       | M    | None       | Backend boots with env vars; clear error when missing |
| T07 | E2   | Implement Auth: Google + Email/Password (Passport.js) | P0       | L    | T06        | Sign in/out works; session persisted                  |
| T08 | E2   | Add user session context to frontend                  | P0       | M    | T07        | User state available across pages via API client      |
| T09 | E2   | Create user profile REST endpoints                    | P1       | M    | T06        | Profile create/read/update works                      |
| T10 | E2   | Create favorites REST endpoints                       | P0       | M    | T06        | Add/remove/list favorites works per user              |
| T11 | E2   | Create watched items REST endpoints                   | P0       | M    | T06        | Add/update/list watched works per user                |
...
| T20 | E5   | Add API ownership guards for user data isolation      | P0       | L    | T10, T11   | User cannot access others' data via API               |
...
- Outcome: user can sign in (via NestJS API) and save favorites/watched end-to-end.
...
- PostgreSQL usage within free-tier limits.
...
- T01 Favorites route.
- T02 Watched route.
- T06 NestJS backend config + env contract.
- T07 Auth integration (Passport.js).
- T10 Favorites REST endpoints + action wiring.
