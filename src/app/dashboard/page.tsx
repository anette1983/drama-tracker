			...
			<main className='flex-1 container mx-auto px-4 py-8'>
				<header className='mb-10'>
					...
					<p className='text-muted-foreground'>
						Continue where you left off in your serenity journey.
					</p>
				</header>
						...
						<section className='p-8 rounded-3xl serenity-gradient border border-primary/20 shadow-sm'>
							...
							<div className='grid sm:grid-cols-2 gap-6'>
								{aiRecs.recommendations.map((rec: any, idx: number) => (
									<Card
										key={idx}
										className='bg-white/60 backdrop-blur-sm border-none shadow-sm hover:shadow-md transition-all'
									>
								...
								{aiRecs.recommendations.length === 0 && (
									<p className='text-muted-foreground col-span-2 text-center py-10'>
										Watching more content to unlock personalized
										recommendations.
									</p>
								)}
							</div>
						...
						<Card className='border-none shadow-md overflow-hidden glass-morphism'>
							...
							<CardContent className='space-y-6'>
								...
								<div className='space-y-4'>
									<h4 className='font-bold text-sm uppercase text-muted-foreground tracking-widest'>
										Recent Reviews
									</h4>
									{[1, 2].map((r) => (
										<div
											key={r}
											className='flex gap-3 items-center p-3 bg-white/50 rounded-xl'
										>
											<div className='w-10 h-10 rounded-lg overflow-hidden relative shrink-0'>
												<Image
													src={`https://picsum.photos/seed/${r + 10}/100/100`}
													alt='Review'
													fill
							...
							<CardContent>
								<div className='space-y-4'>
									{trending.slice(10, 13).map((item) => (
										<div
											key={item.id}
											className='flex items-center gap-3 p-2 hover:bg-primary/5 rounded-xl transition-colors cursor-pointer'
										>
