...
export default async function ContentDetailPage({
	...
	const runtime =
		content.runtime ||
		(content.episode_run_time ? content.episode_run_time[0] : null);
						...
						<div className='flex-1 text-white'>
							<div className='flex flex-wrap gap-2 mb-4'>
								{content.genres?.map((genre: any) => (
									<Badge
										key={genre.id}
										className='bg-accent/80 hover:bg-accent text-white border-none backdrop-blur-md'
									>
										{genre.name}
									</Badge>
								))}
							...
							<div className='flex flex-wrap gap-4'>
								<Button
									size='lg'
									className='bg-accent hover:bg-accent/90 text-white gap-2 rounded-full h-12 px-8'
								>
								...
								<Button
									size='lg'
									variant='outline'
									className='bg-white/10 backdrop-blur-md text-white border-white/30 hover:bg-white/20 gap-2 rounded-full h-12 px-8'
								>
					...
					<div className='lg:col-span-2 space-y-12'>
						...
						<Tabs defaultValue='cast' className='w-full'>
							...
							<TabsContent value='cast' className='mt-8'>
								<div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4'>
									{content.credits?.cast?.slice(0, 8).map((person: any) => (
										<Card
											key={person.id}
											className='bg-white/50 border-none shadow-sm overflow-hidden'
										>
												...
												<Image
													src={
														person.profile_path
															...
															: `https://picsum.photos/seed/${person.id}/300/400`
													}
													alt={person.name}
													fill
													className='object-cover'
							...
							<TabsContent value='recommendations' className='mt-8'>
								{content.recommendations?.results?.length ? (
									<div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4'>
									...
									<div className='text-center py-12 rounded-2xl bg-primary/5 border border-dashed border-primary/20'>
										<p className='text-muted-foreground'>
											No similar Korean or Chinese titles available for this
											item.
										</p>
									</div>
								)}
						...
						<Card className='border-none shadow-md overflow-hidden bg-white/70 backdrop-blur-sm'>
							<CardContent className='p-6 space-y-6'>
								...
								<div className='space-y-4 text-sm'>
									<div>
										<p className='text-muted-foreground mb-1 uppercase tracking-wider text-[10px] font-bold'>
											Status
										</p>
										...
										<div>
											...
											<div className='flex flex-wrap gap-2'>
												{content.networks.map((n: any) => (
													<span key={n.id} className='font-medium'>
														{n.name}
													</span>
												))}
											</div>
									...
									<div>
										...
										<p className='font-medium'>
											{content.original_language === 'ko'
												? 'Korean'
												: content.original_language === 'zh'
													? 'Chinese'
													: content.original_language}
										</p>
									</div>
									{(content.production_companies?.length ?? 0) > 0 && (
										<div>
											<p className='text-muted-foreground mb-1 uppercase tracking-wider text-[10px] font-bold'>
												Production
											</p>
						...
						<Card className='border-none shadow-md overflow-hidden serenity-gradient'>
							<CardContent className='p-6'>
								...
								<div className='space-y-4'>
									...
									<Button className='w-full bg-accent text-white h-11'>
										Save Progress
									</Button>
								</div>
							</CardContent>
